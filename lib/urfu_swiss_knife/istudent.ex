defmodule UrFUSwissKnife.IStudent do
  ...
  def get_subjects(auth, group_id, year, semester) do
    do_get_subjects(auth, group_id, year, semester)
  end
  ...
  defp do_get_subjects(auth, group_id, year, semester) do
    with {:ok, subjects} <- BRS.get_subjects(auth, group_id, year, semester) do
      subjects
  ...
  @decorate cacheable(
              cache: Cache,
              key: {:get_subject_info, auth.username, group_id, year, semester, subject_id},
              ttl: :timer.minutes(15)
            )
  @spec get_subject_info(Token.t(), String.t(), integer(), String.t(), String.t()) ::
          {:ok, SubjectInfo.t()} | {:error, term()}
  def get_subject_info(auth, group_id, year, semester, subject_id) do
    BRS.get_subject(auth, group_id, year, semester, subject_id)
  end
end
